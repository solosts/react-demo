...
// 引入图片
import skyImg from '../../assets/images/login/sky.png';
import earthBgImg from '../../assets/images/login/earth_bg.png';
import cloudImg from '../../assets/images/login/cloud.png';
import starflake1Img from '../../assets/images/login/starflake1.png';
import starflake2Img from '../../assets/images/login/starflake2.png';
...
const loginBg = () => {
  ...
  const initSceneStar = (initZposition: number): any => {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      const y: number = Math.random() * (height / 2);
      const z: number = Math.random() * (zAxisNumber + depth / 2) - depth / 2;
      vertices.push(x, y, z);
    }
  ...
  // 初始化云的运动函数
  const initCloudMove = (cloudParameter: any, speed: number, scaleSpeed = 0.0006, maxScale = 1, startScale = 0) => {
    let cloudProgress = 0;
    return () => {
